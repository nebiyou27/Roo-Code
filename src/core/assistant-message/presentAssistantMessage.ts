		...
		case "tool_use": {
			...
			if (!block.partial) {
				...
				if (!beforeHook.allowed) {
					pushToolResult(formatResponse.toolError(`Tool blocked by intent policy: ${beforeHook.reason}`))
					await hookEngine
						.afterToolUse(
							{
								taskId: cline.taskId,
							...
							{
								status: "denied",
								reason: beforeHook.reason,
								intentId: beforeHook.intentId,
							},
						)
						.catch((error) => console.error("[HookEngine] post-hook failed after deny:", error))
					break
				}
			}

			let hookStatus: "success" | "error" = "success"
			...
			try {
				switch (block.name) {
					...
					case "select_active_intent": {
						const selectedIntentId =
							...
							undefined
						if (!selectedIntentId) {
							pushToolResult(formatResponse.toolError("select_active_intent requires intent_id"))
							break
						}
						pushToolResult(formatResponse.toolResult(`Active intent selected: ${selectedIntentId}`))
						break
					}
					case "new_task":
						await checkpointSaveAndMark(cline)
				...
				hookStatus = "error"
				hookReason = error instanceof Error ? error.message : String(error)
				throw error
			} finally {
				if (!block.partial) {
					await hookEngine
						.afterToolUse(
							...
							},
							{
								status: hookStatus,
								reason: hookReason,
								intentId: hookIntentId,
							},
						)
						.catch((error) => console.error("[HookEngine] post-hook failed:", error))
				}
			}
